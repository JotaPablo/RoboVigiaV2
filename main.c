...
// Estado das máquinas (ligada/desligada)
bool maquinas_estado[2] = {false, true}; // Máquina 1 desligada, Máquina 2 ligada
    ...
    for (int y = 0; y < MAPA_TAM; y++) {
        for (int x = 0; x < MAPA_TAM; x++) {
            ...
            if (x == robo_x && y == robo_y) {
                ...
                // Desenha a máquina 1, cor amarela (ligada ou desligada)
                if (maquinas_estado[0]) {
                    r = 20; g = 20; b = 0; // Mais clara quando ligada
                } else {
                    r = 1; g = 1; b = 0; // Mais escura quando desligada
                }
            }
            else if (mapa[y][x] == MAQUINA_2 && visivel) {
                // Desenha a máquina 2, cor amarela (ligada ou desligada)
                if (maquinas_estado[1]) {
                    r = 20; g = 20; b = 0; // Mais clara quando ligada
                } else {
                    r = 1; g = 1; b = 0; // Mais escura quando desligada
                }
...
//Alarmes para desligar uma maquina especifica
int64_t desliga_maquina_1(alarm_id_t id, void *user_data) {
    
    maquinas_estado[0] = false;
    printf("Máquina 1 foi desligada.\n");
    atualiza_leds_flag = true;

    return 0;  // Não repetirá o alarme
}

int64_t desliga_maquina_2(alarm_id_t id, void *user_data) {
    
    maquinas_estado[1] = false;
    printf("Máquina 2 foi desligada.\n");
    atualiza_leds_flag = true;
    
    return 0;  // Não repetirá o alarme
}

// Função para ligar a maquina por um dos lados(cima, baixo, esquerda ou direita) e configurar o alarme para desligamento
void liga_maquina(int x, int y, int tempo_desligamento_ms) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (adj_x >= 0 && adj_x < MAPA_TAM && adj_y >= 0 && adj_y < MAPA_TAM) {
            // Verifica se há uma máquina na posição adjacente
                if (mapa[adj_y][adj_x] == 1 && !maquinas_estado[0]) { //Se for a maquina 1 e maquina 1 estiver desligada
                        maquinas_estado[0] = true;
                        printf("Máquina em 1 ligada.\n");

                        // Configura o alarme para desligar a máquina após o tempo determinado
                        add_alarm_in_ms(tempo_desligamento_ms, desliga_maquina_1, NULL, false);
                ...
                else if(mapa[adj_y][adj_x] == 2 &&  !maquinas_estado[1]){ //Se for a maquina 2 e maquina 2 estiver desligada
                    maquinas_estado[1] = true;
                    printf("Máquina em 2 ligada.\n");

                    // Configura o alarme para desligar a máquina após o tempo determinado
                    add_alarm_in_ms(tempo_desligamento_ms, desliga_maquina_2, NULL, false);
...
char html[2048]; // Cria a resposta HTML
...
void user_request(char *request) {
    ...
    } else if (strstr(request, "GET /ligarmaquina") != NULL) {
        liga_maquina(robo_x, robo_y, 10000);
    }
...
static err_t tcp_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    "button{border:0;border-radius:8px;padding:12px;margin:4px;font-size:1.1em;background:#ddd;color:#000}"
    ".ctrl{width:25vw;height:25vw;max-width:100px;max-height:100px}"
    ".status{color:%s}"
    "</style></head>"
    "<body><h1>ROBÔ VIGIA</h1>"

    "<div class='info'>"
    "Maquina 1: <strong>%s</strong><br>"
    "Maquina 2: <strong>%s</strong><br>"
    "</div>"

    "<div class='info'>"
    "INTRUSO: <strong class='status'>%s</strong>"
    "</div>"
    ...
    maquinas_estado[0] ? "LIGADA" : "DESLIGADA",
    maquinas_estado[1] ? "LIGADA" : "DESLIGADA",
...
int main()
{
    ...
    add_alarm_in_ms(10000, desliga_maquina_2, NULL, false);
